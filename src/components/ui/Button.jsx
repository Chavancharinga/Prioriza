...
export default function Button({ children, variant = 'primary', disabled = false, loading = false, className = '', ...props }) {
    ...
    return (
        <motion.button
            ...
            className={`inline-flex items-center justify-center gap-2.5 px-5 py-2.5 text-sm font-black tracking-wide uppercase transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary-500/20 disabled:cursor-not-allowed disabled:opacity-50 ${variants[variant]} ${className}`}
            {...props}
        >
